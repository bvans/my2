...
var _ntes_surv = 0;
function _ntes_void() {}
var _ntes_domain_array = ["163.com", "188.com", "netease.com", "126.com", "126.net", "nease.net", "yeah.net", "gz2010.cn", "co188.com", "warcraftchina.com", "youdao.com", "leihuo.net", "gzapg2010.cn", "126.am", "lofter.com", "lede.com", "diablo3.cc", "starcraft2.com.cn", "battlenet.com.cn", "wowchina.com", "hearthstone.com.cn", "heroesofthestorm.com.cn", "bobo.com"];
...
function neteaseTracker(j, a, k, h) {
	var d = h || _ntes_nacc;
	_ntes_nurl = escape(a || document.location);
	_ntes_ntit = escape(k || document.title);
	_ntes_nref = j === true ? "": escape(document.referrer);
	...
	var c = 0;
	for (var e = 0; e < _ntes_domain_array.length; e++) {
		if (_ntes_cdmn == "." + _ntes_domain_array[e]) {
			c = 1;
			break
		}
	}
	if (c == 0) {
		for (e = 0; e < _non_ntes_domain_array.length; e++) {
			if (_non_ntes_cdmn == "." + _non_ntes_domain_array[e]) {
				c = 1;
	...
	if (c == 1) {
		var f = ntes_get_cookie("_ntes_nnid");
		if (f == -1) {
			...
			_ntes_nvid = f.substr(0, m);
			_ntes_surv = f.substr(m + 1, n - m - 1);
			if (_ntes_surv == 0) {
		...
		_ntes_nssn = ntes_get_cookie("P_INFO");
		_ntes_nssn = _ntes_nssn == -1 ? "": _ntes_nssn.substr(0, _ntes_nssn.indexOf("|"));
		_ntes_nstm = ntes_get_cookie("S_INFO") == -1 ? 0 : 1;
		ntes_get_navigation_info();
		var l = ["_nacc=", d, "&_nvid=", _ntes_nvid, "&_nvtm=", _ntes_nvtm, "&_nvsf=", _ntes_nvsf, "&_nvfi=", _ntes_nvfi, "&_nlag=", _ntes_nlag, "&_nlmf=", _ntes_nlmf, "&_nres=", _ntes_nres, "&_nscd=", _ntes_nscd, "&_nstm=", _ntes_nstm, "&_nurl=", _ntes_nurl, "&_ntit=", _ntes_ntit, "&_nref=", _ntes_nref, "&_nfla=", _ntes_flsh, "&_nssn=", _ntes_nssn, "&_nxkey=", (b + "" + Math.random()).substring(6, 20), "&_end1"].join("");
		_ntes_nvsf = 0;
		_ntes_sendInfo("base", _ntes_src_addr + "/ntes?" + l, neteaseTracker.callback);
		neteaseTracker.callback = null;
		if (1) {
			var g = /(?:iPhone|iPad|iPod|Android|Adr |Windows Phone)/i;
			if (_ntes_nacc != "analytics" && _ntes_nacc != "research" && _ntes_nurl.indexOf("https") != 0 && !g.test(navigator.userAgent)) {
				ntes_survey_popup()
			}
		}
	}
...
function neteaseClickStat(b) {
	...
	h = 0;
	for (; h < e; h++) {
		_ntes_bindEvent(g[h], "click", ntes_page_click_stat)
	}
	var k = document.getElementsByTagName("button");
	e = k.length;
	h = 0;
	for (; h < e; h++) {
		_ntes_bindEvent(k[h], "click", ntes_page_click_stat)
	}
	var j = document.getElementsByTagName("input");
	e = j.length;
	h = 0;
	for (; h < e; h++) {
		if (j[h].type == "button" || j[h].type == "submit" || j[h].type == "img") {
			_ntes_bindEvent(j[h], "click", ntes_page_click_stat)
		}
	}
	...
	h = 0;
	for (; h < e; h++) {
		_ntes_bindEvent(a[h], "click", ntes_page_click_stat)
	}
	var d;
	_ntes_bindEvent(window, "scroll", d = function() {
	...
	if (!/compatible/.test(c) && /firefox/.test(c)) {
